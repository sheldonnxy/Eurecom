...
def local_los_probability(city, ue_pose, uav_alt=None, cut_off=0, los_pr_thr=0.5, display=0):
    ...
    #     uav_alt_range = 50
    #     uav_alt_min = city.urban_config['bld_height_max'] + 10
    ...
    # uav_x_range = xy_radius * 2
    # uav_x_min = max(ue_pose[0, 0] - xy_radius, 0)
    # uav_y_range = xy_radius * 2
    # uav_y_min = max(ue_pose[0, 1] - xy_radius, 0)
    #
    # rnd_uav_pos = np.zeros(shape=[num_rnd_pos, 3])
    # uav_elevation = np.zeros(shape=[num_rnd_pos, 1])
    # counter = 0
    # while counter < num_rnd_pos:
    #     rnd_pos = np.random.random(size=(1, 3))
    #     rnd_pos[:, 0] = rnd_pos[:, 0] * uav_x_range + uav_x_min
    #     rnd_pos[:, 1] = rnd_pos[:, 1] * uav_y_range + uav_y_min
    #     rnd_pos[:, 2] = rnd_pos[:, 2] * uav_alt_range + uav_alt_min
    #     if (rnd_pos[:, 0] < city.urban_config['map_x_len']) and (rnd_pos[:, 1] < city.urban_config['map_y_len']):
    #         rnd_uav_pos[counter] = rnd_pos
    #         dist = np.linalg.norm((rnd_pos[0,:2] - ue_pose[0,:2]))
    #         theta = np.arctan(rnd_pos[0, 2] / dist)
    #         uav_elevation[counter] = theta
    #         counter += 1
...
def average_filter(in_data, win_size=10):
    filter_data = []
    for i, _ in enumerate(in_data):
        if i >= win_size:
            filter_data.append(tf.reduce_mean(in_data[i - win_size:i]))
        else:
            filter_data.append(tf.reduce_mean(in_data[:i]))
    return filter_data
