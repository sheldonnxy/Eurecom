import numpy as np
import os
from Defines import city, radio_ch_model, device_position, devices_params, \
    agent_params, uav_start_pose, uav_terminal_pose
...
class DataCollection(EnvironmentBase):
    def __init__(self,
        ...
        self.radio_ch_model = radio_ch_model
        self.link_status = self.init_link_status()
        # self.battery_budget = args.battery_budget

        self.safety_controller_flag = True
        self.env_name = 'UAV_radio'

        # Device information
        self.device_params = devices_params
        self.device_position = device_position.copy()
        self.est_ue_pos = self.device_position.copy()
        self.n_devices = self.device_position.shape[0]
        self.device_list = DeviceList(position=self.device_params['position'].copy(), color=self.device_params['color'].copy(),
                                      data=self.device_params['data'].copy(), user_num=self.device_params['user_num'])
        ...
        self.start_pose = agent_params['start_pose'][0:args.n_agents].copy()
        self.end_pose = agent_params['end_pose'][0:args.n_agents].copy()
        self.battery_budget = agent_params['battery_budget'].copy()
        self.n_agents = args.n_agents
        # initialize agents
        self.agents = [AgentModel(agent_params['start_pose'][i], agent_params['end_pose'][i],
                                  self.battery_budget[i]) for i in range(self.n_agents)]
        ...
        self.episode_limit = int(max(agent_params['battery_budget']) * 2)
        ...
        self.dis_scale = max(self.max_index_x, self.max_index_y) * self.step_size
        self.battery_scale = max(agent_params['battery_budget'])
    ...
    def reset(self, model=False):
        # reset the user position according to the model
        if model:
            self.set_est_ue_pos()
        else:
        ...
        for a_id, action in enumerate(actions):
            ...
            if agent.battery > 0 and agent.done is False:
                ...
                if device_id != -1:
                    ...
                    if model:
                        collected_meas = self.learning_channel_model.get_user_capacity(agent.current_pose, device.position)
    ...
    def get_obs_agent(self, agent_id):
        ...
        # if the UAV still has enough battery and doesn't finish the task
        ...
        if agent.battery > 0 and agent.done is False:
            ...
            for d_id, device in enumerate(self.device_list.devices):
                ...
                # the signal strength is enough to let the UAV get access to the device
                if snr_db >= self.device_sight_range:
                    device_feats[d_id, 0] = 1   # whether the UAV can access this device
    ...
    def get_state_dict(self):
        ...
        for de_id, device in enumerate(self.device_list.devices):
            est_ue_pos = self.est_ue_pos[de_id].copy()
    ...
    def get_agent_device_snr(self, agent_id, model=False):
        """ Return a numpy array of the SNR of each device for agent"""
    ...
    def get_avail_devices(self):
        ...
        for i in range(len(snr_array)):
            ...
            device_access[agent_index] = user_index
            snr_array[agent_index, :] = -np.inf
            snr_array[:, user_index] = -np.inf
    ...
    def set_est_ue_pos(self):
        ''' Update the position of the users for model-aided learning '''
        self.device_position = self.est_ue_pos.copy()
        self.device_list.update_position(self.device_position)

    def set_default_position(self):
        self.device_position = self.device_params['position'].copy()
        self.device_list.update_position(self.device_position)

    def set_device_position(self, est_ue_pos):
        self.est_ue_pos = est_ue_pos.copy()
        self.set_est_ue_pos()
